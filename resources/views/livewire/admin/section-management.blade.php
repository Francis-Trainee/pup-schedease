    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl p-6 z-10">
            ...
            <form wire:submit.prevent="{{ $isEdit ? 'update' : 'save' }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <input wire:model.defer="program" type="text" placeholder="Program" class="border rounded-lg px-3 py-2 w-full">
                    @error('program') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>

                <div>
                    <input wire:model.defer="year_level" type="text" placeholder="Year Level" class="border rounded-lg px-3 py-2 w-full">
                    @error('year_level') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>

                <div>
                    <input wire:model.defer="section_name" type="text" placeholder="Section Name" class="border rounded-lg px-3 py-2 w-full">
            ...
            <tbody>
                ...
                <tr class="border-b">
                    ...
                    <td class="flex gap-2 px-6 py-3">
                        <button wire:click="edit({{ $section->id }})" class="text-blue-600 hover:underline">Edit</button>
                    </td>
                </tr>
                @endforeach
            </tbody>
