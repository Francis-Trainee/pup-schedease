    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl p-6 z-10">
            ...
            <form wire:submit.prevent="{{ $isEdit ? 'update' : 'save' }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <input wire:model.defer="room_name" type="text" placeholder="Room Name" class="border rounded-lg px-3 py-2 w-full">
                    @error('room_name')
                ...
                <div>
                    <input wire:model.defer="room_type" type="text" placeholder="Room Type (Lecture/Lab)" class="border rounded-lg px-3 py-2 w-full">
                    @error('room_type')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
                </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6 z-10">
            ...
            <div class="flex justify-end gap-2">
                ...
                <button wire:click="deactivateRoom" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg">
                    Confirm Deactivate
                </button>
            </div>
            ...
            <tbody>
                ...
                <tr class="border-b">
                    ...
                    <td class="px-6 py-3 whitespace-nowrap">
                        <span class="px-2 py-1 rounded-full text-xs {{ $room->status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                            {{ ucfirst($room->status) }}
                        </span>
                    </td>
                    <td class="flex gap-2 px-6 py-3">
                        <button wire:click="edit({{ $room->id }})" class="text-blue-600 hover:underline">Edit</button>
